// VulkanApp.cpp : Definiert den Einstiegspunkt für die Anwendung.
//
...
struct Demo {
	...
	void create_device() {
		...
		device.createFence(&fence_ci, nullptr, &fences[0]);
		bool test = true;
	...
	void draw() {
		...
		vk::CommandBuffer * cmd_test = &buffers[current_buffer].cmd;
	...
	void prepare_descriptor_set() {
		...
		write_uniform.setDstSet(desc_set);
		write_uniform.setDstBinding(0);
		write_uniform.setDescriptorCount(1);
		write_uniform.setDescriptorType(vk::DescriptorType::eUniformBuffer);
		write_uniform.setPBufferInfo(&uniform_data.buffer_info);

		device.updateDescriptorSets(1, &write_uniform , 0, nullptr);


		vk::WriteDescriptorSet write_image;

		write_image.setDstSet(desc_set);
		write_image.setDstBinding(1);
		write_image.setDescriptorCount(texture_count);
		write_image.setDescriptorType(vk::DescriptorType::eCombinedImageSampler);
		write_image.setPImageInfo(tex_descs);

		device.updateDescriptorSets(1, &write_image , 0, nullptr);
	}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine,
	...
	//Graphic::GraphicManager * manager = Graphic::GraphicManager::getSingletonPtr();
	//manager->createInstance();
	//manager->getPhysicalDevice();

	demo.init(argc, argv);

	//manager->setInstance(&demo.inst);
	//manager->setPhysicalDevice(&demo.gpu);
	...
	// Test Version 
	Graphic::Win32Surface surface;
	surface.connection = hInstance;
	surface.window = demo.window;
	surface.width = 500;
	surface.height = 500;

	//Graphic::GraphicManager * manager = Graphic::GraphicManager::getSingletonPtr();
	//manager->createInstance();
	//manager->createSurface(&surface);
	//manager->getPhysicalDevice();
